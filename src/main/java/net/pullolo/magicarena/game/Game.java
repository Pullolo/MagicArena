...
public class Game {
        ...
        BukkitRunnable gameClock1s = new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (!isTeamAlive(team1) && isTeamAlive(team2)){
                    gameClock1t.cancel();
                    finishGame(team2, team1, allPlayers, gameType);
                    cancel();
                    return;
                }
                if (isTeamAlive(team1) && !isTeamAlive(team2)){
                    gameClock1t.cancel();
                    finishGame(team1, team2, allPlayers, gameType);
                    cancel();
                    return;
                }
                if (!isTeamAlive(team1) && !isTeamAlive(team2)){
                    gameClock1t.cancel();
                    finishGame(null, null, allPlayers, gameType);
                    cancel();
                    return;
                }
            }
    ...
    public void finishGame(ArrayList<Player> winners, ArrayList<Player> losers, ArrayList<Player> allPlayers, QueueManager.QueueType gameType){
        for (Player p : allPlayers){
            if (p!=null){
